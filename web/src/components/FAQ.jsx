...
const faqs = [
  { q: 'What makes Kode different from Cursor/Copilot?', a: 'Every other agent uses generate-and-pray — the LLM generates code and writes it directly to disk. Kode runs 6 deterministic verification gates between the LLM and your filesystem. If any gate fails, the patch is rejected and the LLM self-corrects. The user is never the verification layer.' },
  { q: 'What are the 6 verification gates?', a: 'Syntax (Tree-sitter AST parsing for 5 languages), Imports (dependency graph validation), Calls (hallucinated function detection), Blast Radius (downstream impact analysis), Architecture (module boundary enforcement), and Security (vulnerability scanning). All run in a compiled Go binary in under 50ms.' },
  ...
  { q: 'What is Kode?', a: 'Kode is an open source AI coding agent with a compiled Go verification engine. Every generated patch must pass 6 deterministic gates before touching your filesystem. It supports a full Plan → Critique → Generate → Verify → Apply → Test → Bench pipeline with automatic rollback.' },
