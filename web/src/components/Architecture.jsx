export default function Architecture() {
  const stages = [
    ...
    { name: 'Verify', desc: '6 gates check every file before disk write', active: true },
      ...
      <div className="wrapper">
        <div className="features-list">
          ...
          <p className="body-md" style={{ marginBottom: 8 }}>
            The Go engine orchestrates a 7-stage pipeline for every task. The <strong>Verify</strong> stage is the thesis — a compiled Go binary that runs 6 inline gates before any file write is permitted.
          </p>
          <div className="arch-flow">
            ...
            <div className="gate-note">
              <strong>How it differs:</strong> Cursor and Copilot run <code>LLM → disk</code> (pray it works). Kode runs <code>LLM → 6 gates → disk</code> (verified) — or <code>fail → LLM self-corrects → retry</code>. The user is never the verification layer.
            </div>
          </div>
        </div>
      </div>
