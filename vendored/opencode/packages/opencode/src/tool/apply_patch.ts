...
import { VerificationGatekeeper } from "../bridge/gatekeeper"
      ...
      if (verifyFiles.length > 0) {
        ...
        const { approved, result } = yield* Effect.promise(() =>
          ...
          Effect.catchAll((error) =>
            Effect.succeed({
              approved: false,
              result: { status: "FAIL" as const, failed_files: { gatekeeper: error instanceof Error ? error.message : String(error) } },
            }),
          ),
        )
        if (!approved) {
          ...
          for (const [filePath, reason] of Object.entries(result.failed_files)) {
            lines.push(`  ✗ ${path.relative(instance.worktree, filePath)}`)
            lines.push(`      ${reason}`)
          }
          const output = `Kode Gate blocked the patch:\n\n${lines.join("\n")}\n\nUse Force to apply anyway, or retry with fixes.`
          yield* ctx.metadata({ title: "🛑 Gate Blocked", metadata: { verdict: result, files } })
          return { title: output, metadata: { verdict: result, files }, output }
        }
      }
