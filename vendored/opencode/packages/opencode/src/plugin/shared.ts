...
export async function checkPluginCompatibility(target: string, kodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(kodeVersion) || semver.major(kodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.kode
  if (typeof range !== "string") return
  if (!semver.satisfies(kodeVersion, range)) {
    throw new Error(`Plugin requires kode ${range} but running ${kodeVersion}`)
  }
}
