...
const TUI_SCHEMA_URL = "https://kode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from kode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const files = await kodeFiles(input)
...
async function kodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "kode"),
    ...(await Filesystem.findUp(["kode.json", "kode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "kode"))
  }
  if (Flag.KODE_CONFIG) files.push(Flag.KODE_CONFIG)
