...
import { AISDK } from "@kode/core/aisdk"
import { ModelV2 } from "@kode/core/model"
import { PluginV2 } from "@kode/core/plugin"
import { DynamicProviderPlugin } from "@kode/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "kode-provider-dynamic-"))
