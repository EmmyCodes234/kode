...
import { EventV2 } from "@kode/core/event"
import { it } from "./lib/effect"
import { rm, writeFile, utimes, mkdir } from "fs/promises"
import path from "path"

// test/preload.ts pins KODE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.KODE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.KODE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.KODE_MODELS_PATH = undefined
  Flag.KODE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.KODE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.KODE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
