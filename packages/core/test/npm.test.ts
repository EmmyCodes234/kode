...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@kode/acme")).toBe("@kode/acme")
    expect(Npm.sanitize("@kode/acme@1.0.0")).toBe("@kode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/kode/acme.git"
    const expected = win ? "acme@git+https_//github.com/kode/acme.git" : spec
