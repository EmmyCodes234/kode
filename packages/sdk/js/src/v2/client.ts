...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-kode-directory", "directory"],
    ["x-kode-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-kode-directory")
  next.headers.delete("x-kode-workspace")
  return next
}
...
export function createKodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-kode-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-kode-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of kode Server (Server responded with text/html)")
