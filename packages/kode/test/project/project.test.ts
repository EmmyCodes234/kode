...
import { Project as ProjectV2 } from "@kode/core/project"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const KodeFile = path.join(tmp, ".git", "kode")
      expect(yield* Effect.promise(() => Bun.file(KodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "kode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "kode")
      const wrongCache = path.join(parentDir, ".git", "kode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "kode")
      const cacheB = path.join(bareB, "kode")
      const wrongCache = path.join(parentDir, ".git", "kode")
