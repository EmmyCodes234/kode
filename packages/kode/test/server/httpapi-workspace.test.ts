...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-kode-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://trykode.xyz/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-kode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-kode-workspace")
      } finally {
