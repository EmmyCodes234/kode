...
// terminal sizes. The default kode tui command is excluded —
// `kode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `kode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real kode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("kode CLI help-text snapshots", () => {
    ...
    ({ kode }) =>
      Effect.gen(function* () {
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* kode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`kode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`kode ${argv.join(" ")} --help`)
        }
