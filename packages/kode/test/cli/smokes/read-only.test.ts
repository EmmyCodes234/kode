...
describe("kode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // KODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ kode }) =>
      Effect.gen(function* () {
        const r = yield* kode.spawn(["mcp", "list"])
        kode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ kode }) =>
      Effect.gen(function* () {
        const r = yield* kode.spawn(["providers", "list"])
        kode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ kode }) =>
      Effect.gen(function* () {
        const r = yield* kode.spawn(["models"])
        kode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ kode }) =>
      Effect.gen(function* () {
        const r = yield* kode.spawn(["agent", "list"])
        kode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh KODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ kode }) =>
      Effect.gen(function* () {
        const r = yield* kode.spawn(["session", "list"])
        kode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ kode }) =>
      Effect.gen(function* () {
        const r = yield* kode.spawn(["stats"])
        kode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under KODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ kode }) =>
      Effect.gen(function* () {
        const r = yield* kode.spawn(["db", "path"])
        kode.expectExit(r, 0, "db path")
