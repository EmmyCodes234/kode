...
it.instance("plan agent denies edits except .kode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".kode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".kode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.KODE_TEST_HOME
      process.env.KODE_TEST_HOME = test.directory
