...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "kode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "kode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["kode.json", "kode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "kode.json"),
        path.join(tmp.path, "kode.jsonc"),
        path.join(project, "kode.json"),
        path.join(project, "kode.jsonc"),
      ])
    })
  })
