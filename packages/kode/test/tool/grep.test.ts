  ...
  it.instance("does not ask for external_directory when alias path is allowed", () =>
    Effect.gen(function* () {
      ...
      const tmp = yield* Effect.acquireRelease(
        Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "kode-grep-alias-"))),
  ...
  scout.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cache = path.join(Global.Path.repos, "github.com", "kode-grep-reference", "repo")
        ...
        const remoteDir = path.join(remoteRoot, "kode-grep-reference")
