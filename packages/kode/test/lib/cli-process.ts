// Subprocess test harness for the kode CLI. Spawns the real binary against
...
//
// Configuration flows through kode's built-in test affordances:
//   - KODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - KODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - KODE_DISABLE_PROJECT_CONFIG : skip walking up for kode.json
//   - KODE_PURE                : skip external plugin discovery + install
//   - KODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `kode.run` is fully wired. The shape supports adding more
// builders (`kode.serve(opts)`, `kode.acp(opts)`, `kode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of KodeCli.
import { test, type TestOptions } from "bun:test"
import { AppFileSystem } from "@kode/core/filesystem"
import { AppProcess } from "@kode/core/process"
...
const KodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(KodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    KODE_CONFIG_CONTENT: configJson,
    KODE_DISABLE_PROJECT_CONFIG: "1",
    KODE_PURE: "1",
    KODE_DISABLE_AUTOUPDATE: "1",
    KODE_DISABLE_AUTOCOMPACT: "1",
    KODE_DISABLE_MODELS_FETCH: "1",
    KODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `kode run`. New flags should
...
// `kode serve` is a long-lived process — it never exits on its own.
...
// `kode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type KodeCli = {
  ...
  // Spawn `kode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `kode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("kode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("kode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "kode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `kode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("kode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const kode: KodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, kode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `kode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "kode") {
...
// every call site. Use this for any test that needs the kode CLI fixture.
//
...
// resources (e.g. `kode.serve`) without an extra `Effect.scoped` wrapper —
