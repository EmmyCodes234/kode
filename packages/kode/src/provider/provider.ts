...
import { Hash } from "@kode/core/util/hash"
import { Plugin } from "../plugin"
import { serviceUse } from "@kode/core/effect/service-use"
import { type LanguageModelV3 } from "@ai-sdk/provider"
import * as ModelsDev from "@kode/core/models-dev"
...
import { optionalOmitUndefined } from "@kode/core/schema"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@kode/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    openmodel: Effect.fnUntraced(function* (input: Info) {
      const env = yield* dep.env()
      const key = env["OPENMODEL_API_KEY"]
      input.env = [...new Set([...input.env, "OPENMODEL_API_KEY"])]

      const models: Record<string, Model> = {
        "deepseek-v4-flash": {
          id: ModelID.make("deepseek-v4-flash"),
          providerID: ProviderID.make("openmodel"),
          name: "DeepSeek V4 Flash",
          family: "deepseek-v4",
          api: { id: "deepseek-v4-flash", url: "https://api.openmodel.ai/v1", npm: "@ai-sdk/anthropic" },
          capabilities: {
            temperature: true, reasoning: false, attachment: true, toolcall: true,
            input: { text: true, audio: false, image: false, video: false, pdf: false },
            output: { text: true, audio: false, image: false, video: false, pdf: false },
          ...
          cost: { input: 0.035, output: 0.07, cache: { read: 0, write: 0 } },
          limit: { context: 65536, output: 8192 },
          status: "active", options: {}, headers: {}, release_date: "", variants: {},
        },
      }
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from kode.json provider config
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `Kode/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `Kode/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `kode auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `Kode/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
...
export class Service extends Context.Service<Service, Interface>()("@kode/Provider") {}
...
const priority = ["gpt-5", "claude-sonnet-4", "deepseek-v4-flash", "gemini-3-pro"]
