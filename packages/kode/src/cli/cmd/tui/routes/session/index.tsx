...
import { KODE_BASE_MODE, useBindings, useCommandShortcut, useKodeKeymap } from "../../keymap"
...
const GO_UPSELL_PROVIDERS = new Set(["kode", "kode-go"])
...
const sessionBindingCommands = [
  // "session.share",  // Disabled: Kode does not operate a session sharing server
  "session.rename",
  "session.timeline",
  "session.fork",
  "session.compact",
  // "session.unshare",  // Disabled: Kode does not operate a session sharing server
  "session.undo",
  "session.redo",
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}kode -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  const sessionCommandList = createMemo(() => [
    {
      ...
      // Sharing would attempt to call upstream OpenCode infrastructure.
      title: "Share session (not available)",
      value: "session.share",
      suggested: false,
      category: "Session",
      enabled: false,
      ...
      run: async () => {
        toast.show({
          message: "Session sharing is not available in Kode. Kode operates 100% locally.",
          variant: "warning",
          duration: 4000,
        })
        dialog.clear()
    ...
    {
      // Disabled: Kode does not operate a session sharing server.
      title: "Unshare session (not available)",
...
function ReasoningPart(props: { last: boolean; part: ReasoningPart; message: AssistantMessage }) {
  ...
  // OpenAI / Copilot / kode-via-OpenAI emit `**Title**\n\n<body>` summary
