...
export const KODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const KODE_MODE_KEY = "kode.mode"

export const KodeKeymapProvider = KeymapProvider
export const useKodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type KodeModeStack = ReturnType<typeof createKodeModeStack>
...
export function createKodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(KODE_MODE_KEY, KODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(KODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(KODE_MODE_KEY, stack.at(-1)?.mode ?? KODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? KODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(KODE_MODE_KEY, undefined)
...
export function useKodeModeStack() {
  return getKodeModeStack(useKodeKeymap())
}

export function getKodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Kode mode stack is not registered for this keymap")
  return value
}
...
export function registerKodeKeymap(
  ...
  const modeStack = createKodeModeStack(keymap)
