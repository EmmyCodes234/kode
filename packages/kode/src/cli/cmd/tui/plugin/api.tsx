import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@kode/plugin/tui"
...
import type { useKodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useKodeKeymap>
...
export function createTuiApi(input: Input): TuiPluginApi {
  ...
  return {
    ...
    mode: {
      current() {
        return Keymap.getKodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getKodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
