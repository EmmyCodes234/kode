...
//      kode server and runs interactive mode against it.
//
...
import { createKodeClient, type KodeClient, type ToolPart } from "@kode/sdk/v2"
...
type ModelInput = Parameters<KodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run kode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("share", {
        type: "boolean",
        describe: "share the session (not available in Kode)",
        hidden: true,
      })
      .option("model", {
        type: "string",
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running kode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to KODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to KODE_SERVER_USERNAME or 'kode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: KodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      // Disabled: Kode does not operate a session sharing server.
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      async function share(_sdk: KodeClient, _sessionID: string) {
        // No-op. Kode operates 100% locally and does not support session sharing.
        return
      }
      ...
      async function current(sdk: KodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: KodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: KodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: KodeClient) {
        ...
        async function loop(client: KodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createKodeClient({
        baseUrl: "http://kode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
