...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@kode/llm"
import type { LLMClientShape } from "@kode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("kode"))
    return { type: "unsupported", reason: "provider is not openai, kode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @kode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains Kode-owned. The native runtime only adapts
      // the @kode/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
