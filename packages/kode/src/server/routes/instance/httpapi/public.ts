...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for kode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(KodeHttpApi)
...
export const PublicApi = KodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "kode",
    version: "1.0.0",
